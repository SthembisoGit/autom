  ...
  async synthesize(
    ...
    for (const [index, chunk] of narrationChunks.entries()) {
      ...
      try {
        ...
        if (isAbortError(error)) {
          throw new Error(
            `Deepgram narration request timed out while synthesizing chunk ${index + 1} of ${narrationChunks.length}.`
          );
        }
      ...
      try {
        ...
        if (isAbortError(error)) {
          throw new Error(
            `Deepgram narration response timed out while reading chunk ${index + 1} of ${narrationChunks.length}.`
          );
        }
    ...
    return {
      ...
      sceneNarrationTimeline: buildSceneNarrationTimeline(
        ...
        estimateNarrationDurationSeconds(scriptPackage.scenes.map((scene) => scene.text))
      ),
    };
  }
}
